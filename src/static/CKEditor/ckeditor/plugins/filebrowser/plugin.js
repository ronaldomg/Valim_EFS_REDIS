 ...
 * For licensing, see LICENSE.md or http://ckeditor.com/licens
 *
/*
 * @fileOverview The "filebrowser" plugin that adds support for file uploads an
 *               browsing
 
 * When a file is uploaded or selected inside the file browser, its URL i
 * inserted automatically into a field defined in the <code>filebrowser</code
 * attribute. In order to specify a field that should be updated, pass the tab ID an
 ...
 * 	label : editor.lang.common.browseServe
 * 
 * </pre
 
 * If you set the <code>filebrowser</code> attribute for an element other tha
 ...
 * The filebrowser plugin also supports more advanced configuration performed throug
 * a JavaScript object
 ...
 * 	filebrowser 
 * 	
 * 		action : 'QuickUpload', // require
 * 		target : 'tab1:elementId', // require
 * 		params : // optiona
 * 		
 * 			type : 'Files'
 * 			currentFolder : '/folder/
 * 		}
 * 		onSelect : function( fileUrl, errorMessage ) // optiona
 * 		
 ...
 * Suppose you have a file element with an ID of <code>myFile</code>, a tex
 ...
 * or, if not specified, to <code>filebrowserUploadUrl</code>. Additional parameter
 * from the <code>params</code> object will be added to the query string. It i
 * possible to create your own <code>uploadHandler</code> and cancel the built-i
 ...
 * In this example, when the button is pressed, the file browser will be opened in 
 * popup window. If you do not specify the <code>filebrowser.url</code> attribute
 * <code>filebrowser[<em>DialogName</em>]BrowseUrl</code> o
 * <code>filebrowserBrowseUrl</code> will be used. After selecting a file in the fil
 * browser, an element with an ID of <code>elementId</code> will be updated. Jus
...
( function() 
	...
	// The onlick function assigned to the 'Browse Server' button. Opens th
	...
	/
	// @since 4.5.
	// @param {CKEDITOR.dom.element} fileInpu
	...
	// The onlick function assigned to the 'Upload' button. Makes the fina
	// decision whether form is really submitted and updates target field whe
	// file is uploaded
	/
	// @param {CKEDITOR.event
	//            evt The event object
	...
	// @param {Object
	//            filebrowser Object containing filebrowser settings assigned t
	...
	// Traverse through the content definition and attach filebrowser t
			...
			if ( element.filebrowser.action == 'Browse' ) 
				...
				if ( url ) 
					...
					element.onClick = function( evt ) 
						// "element" here means the definition object, so we need to find the correc
						// button to scope the event cal
 ...
 * button is pressed. If configured, the **Browse Server** button will appear in th
 ...
 *
/*
 * The width of the file manager popup window. It can be a number denoting a value i
 ...
 *
/*
 * The height of the file manager popup window. It can be a number denoting a value i
 * pixels or a percent string
